...
public class MotionProfileExample {
	...
	private void startFilling() {
		startFilling(_profileString);
	}

	private void startFilling(String profile) {
		CSVArrays profileArrays = new CSVArrays(profile);
		int totalCnt = profileArrays.getAcc().length;
		...
		double finalPositionRot = profileArrays.getPos()[profileArrays.getPos().length-1];
		
		/* This is fast since it's just into our TOP buffer */
		for (int i = 0; i < totalCnt; ++i) {
			double direction = _bForward ? +1 : -1;
			double positionRot = profileArrays.getPos()[i];
			double velocityRPM = profileArrays.getVel()[i];
			...
			point.timeDur = GetTrajectoryDuration((int)profileArrays.getDt()[i]);
			point.zeroPos = false;
			if (i == 0)
